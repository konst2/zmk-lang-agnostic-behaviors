...
| zmk,behavior-kp-on-lang | Символ | Нажатие символа только в заданной раскладке. Переключает раскладку если нужно. Использовать для симвлолов типа '#', "№", которые есть только на одной раскладке |
| zmk,behavior-ru-agnostic | Символ | Пытается найти символ в текущей раскладке системы и нажать соответствующую клавишу. Использовать для символов типа '?', '.', которые присутствуют в разных раскладках, но на разных клавишах |
...
/ {
    behaviors {
        ...
        // разобраться, где на клавиатуре этот символ в зависимости от текущей раскладки
        // и нажать его именно там, без переключения раскладки в системе
        // использовать только для символов которые переезжают на другую клавишу
        // в русской раскладке, типа ".", "?" и т.д.
        agn: ru_agnostic {
            compatible = "zmk,behavior-ru-agnostic";
            #binding-cells = <1>;
            en_layer = <ENGLISH>;
            ru_layer = <RUSSIAN>;
        };
...
```
&en HASH   // Всегда вводит '#', даже если активна русская раскладка с переключением раскладки
&ru HASH   // Всегда вводит '№', даже если активна английская раскладка с переключением раскладки
&agn &agn QUESTION  // всегда выводит '?' без переключения раскладки -- этот символ должен присутствовать в обоих раскладках
```
